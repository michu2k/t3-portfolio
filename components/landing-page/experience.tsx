  ...
  return (
    <li className="group flex flex-col items-start gap-2 md:flex-row md:gap-8">
      <div className="flex flex-col justify-center md:min-h-[2rem]">
        <span className="flex-inline text-muted-foreground min-w-[10rem] font-poppins text-sm font-semibold transition-colors group-hover:text-primary">
          ...
          <ul className="flex flex-col gap-2">
            {responsibilities.map(({id, name}) => (
              <li key={id} className="flex items-start gap-3">
                ...
                <p className="text-md text-muted-foreground leading-7">{name}</p>
              </li>
            ))}
          </ul>
        ) : null}
