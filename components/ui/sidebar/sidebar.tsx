...
type SidebarContentProps = PropsWithChildren<{
  className?: string;
  /** If set to `false`, a standard sidebar will be displayed */
  isExpandable?: boolean;
}>;

const SidebarContent = React.forwardRef<HTMLElement, SidebarContentProps>(
  ({children, isExpandable = true, className}, ref) => {
    ...
    const sidebarClassName = cn(
      "flex shrink-0 flex-col h-full py-10 px-4 bg-background border-r-[1px] border-muted w-full max-w-[18rem] md:w-72 md:h-screen z-50 top-0 fixed md:sticky",
      className
    );
