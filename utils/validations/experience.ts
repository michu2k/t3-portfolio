...
const experienceItemResponsibilitySchema = z.object({
  ...
  name: z.string().max(64, "Name is too long")
});

const experienceItemSchema = z
  .object({
    id: z.string().optional(),
    position: z.string().min(3, "Position name must be at least 3 characters long").max(64, "Name is too long"),
    company: z.string().min(3, "Company name must be at least 3 characters long").max(64, "Name is too long"),
