...
export const socialMediaRouter = createTRPCRouter({
  ...
  createItem: protectedProcedure.input(socialMediaLinkSchema).mutation(async ({ ctx, input }) => {
    return await ctx.prisma.socialMediaLink.create({
      data: input
    });
  }),

  updateItem: protectedProcedure.input(socialMediaLinkSchema).mutation(async ({ ctx, input: { id, ...input } }) => {
