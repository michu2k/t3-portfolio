...
export const contactRouter = createTRPCRouter({
  ...
  createItem: protectedProcedure.input(contactMethodSchema).mutation(async ({ ctx, input }) => {
    return await ctx.prisma.contactMethod.create({
      data: input
    });
  }),

  updateItem: protectedProcedure.input(contactMethodSchema).mutation(async ({ ctx, input: { id, ...input } }) => {
